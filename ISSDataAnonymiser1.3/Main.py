...
last_col = does_exist(ws, "Is there a LAST name column?")
email_col = does_exist(ws, "Is there an EMAIL column?")
...
if os.path.exists("email_map.json"):
    with open("email_map.json", "r") as f:
        email_map = json.load(f)

# Set counters based on existing data
counter = len(name_map) + 1
ecounter = len(email_map) + 1

for row in range(2, ws.max_row + 1):  # skip header row
    first = ws[f"{first_col}{row}"].value
    last = ws[f"{last_col}{row}"].value if last_col else None
    email = ws[f"{email_col}{row}"].value if email_col else None
    ...
    if first and not last:
        name = f"{str(first).strip()}"
        
        if name not in name_map:
            name_map[name] = f"Person_{counter:03}"
            counter += 1
        
        first_anon_value = name_map[name]
        ws[f"{first_col}{row}"] = first_anon_value
